...
def getWatchedSig():
    ...
    if key:
        try:
            current_ip = com.get_public_ip()
            now = int(time.time()) * 1000
            jkey = json.loads(json.loads(base64.b64decode(key.encode('utf-8')).decode('utf-8'))['data'])
            key_ip = jkey.get('ips', [None])[0]
            key_ok = jkey.get('app', {}).get('ok')
            valid = int(jkey.get('validUntil', 0))
            if current_ip == key_ip and key_ok is True and valid > now:
                return key
        except Exception:
            pass
    ...
    try:
        req = requests.post('https://www.lokke.app/api/app/ping', json=_data, headers=_headers, timeout=20).json()
        sig = req.get("addonSig") or req.get("data", {}).get("addonSig")
    except Exception:
        sig = None
...
def resolve_link(link):
    _headers={"user-agent": "MediaHubMX/2", "accept": "application/json", "content-type": "application/json; charset=utf-8", "accept-encoding": "gzip", "mediahubmx-signature": getWatchedSig()}
